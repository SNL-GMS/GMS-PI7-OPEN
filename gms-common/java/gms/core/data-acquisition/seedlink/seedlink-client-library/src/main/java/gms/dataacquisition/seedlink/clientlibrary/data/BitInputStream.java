...
/**
 * An input stream that allows variable-bit length integers to be read from the provided
 * input stream.  Do not use this input stream for any other reading as it performs
 * internal buffering of the bytes.
 *
...
public class BitInputStream extends BufferedInputStream
{
  ...
  /**
   * Construct a bit input stream over the provided input stream,
   * reading N bytes at a time.
   *
   * @param is
   * @param N
   */
  public BitInputStream(InputStream is, int N)
  {
    super(is, N);
  }

  /**
   * Read a 32 bit integer constructed from the next n bits of 2's complement
   * data.
   *
