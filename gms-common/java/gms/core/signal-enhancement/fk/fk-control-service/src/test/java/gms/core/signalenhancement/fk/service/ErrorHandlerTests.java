...
public class ErrorHandlerTests {
  ...
  public void testHandle400BadRequest() {
    final String errorMessage = "Custom error message causing the 400";
    final String expectedBody = "Error 400 - Bad Request: " + errorMessage;

    StandardResponse response = ErrorHandler.handle400(errorMessage);
    assertNotNull(response);
    assertEquals(HttpStatus.BAD_REQUEST_400, response.getHttpStatus());
    assertEquals(ContentType.TEXT_PLAIN, response.getContentType());
    assertEquals(expectedBody, response.getResponseBody());
  }

  @Test
  public void testHandle400ErrorMessageEmpty() {
    final String expectedBody = "Error 400 - Bad Request";
    StandardResponse response = ErrorHandler.handle400(" ");
    assertNotNull(response);
    assertEquals(HttpStatus.BAD_REQUEST_400, response.getHttpStatus());
    assertEquals(ContentType.TEXT_PLAIN, response.getContentType());
    assertEquals(expectedBody, response.getResponseBody());
  ...
  @Test
  public void testHandle400NullMessageExpectNullPointerException() {
    exception.expect(NullPointerException.class);
    exception.expectMessage("ErrorHandler.handle400 requires non-null errorMessage");
    StandardResponse response = ErrorHandler.handle400(null);
  }
