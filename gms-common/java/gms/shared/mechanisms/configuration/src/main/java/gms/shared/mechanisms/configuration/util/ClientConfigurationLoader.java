...
/**
 * Creates {@link CompositeConfiguration} based on a hierarchy of: Environment variables System
 * properties (i.e. as set with java -D command line flags) properties file
 *
 * Settings in higher levels override properties from lower levels.
 */
public class ClientConfigurationLoader {

  private static Logger logger = LoggerFactory.getLogger(ClientConfigurationLoader.class);
   ...
   * @throws RuntimeException if the propertiesFileUrl cannot be loaded into a {@link
   * Configuration}
   */
  public static CompositeConfiguration load(URL propertiesFileUrl) {

    Objects.requireNonNull(propertiesFileUrl,
        "ClientConfigurationLoader requires non-null propertiesFileUrl");
    ...
    try {
      ...
      throw new RuntimeException(
          "ClientConfigurationLoader could not load configuration file " + propertiesFileUrl, e);
    }
