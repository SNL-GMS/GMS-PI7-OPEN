...
public class VectorUnit {
   ...
   * return errorValue
   * @return the azimuth from v1 to v2, in radians clockwise from north, or errorValue.  Range is
   * -PI to PI
   */
   ...
   * @throws IOException if v0 and v1 are parallel.
   */
   ...
   * passed as an argument to this method. The second is located 90 degrees away from v in the
   * direction specified by azimuth.
   * @throws IOException if v is located at north or south pole.
   */
   ...
   * @param x the position to be moved.
   * @param distance the distance, in radians, that x is to be moved toward the north.
   * @param z the 3-element unit vector representing the position after having moved distance
   * north.
  ...
  /**
   * Rotate unit vector x clockwise around unit vector p, by angle a. Angle a is in radians and
   * rotation is clockwise when viewed from outside the unit sphere.
   * <p>
   ...
   * @param ang The rotation angle (from p0 toward y).
   * @param z The rotated vector of unit length.
   */
  ...
  /**
   * Compute the normalized vector triple product (v0 x v1) x v0 and and store result in rslt.  It
   * is ok if rslt is a reference to v1 or v2.  Local variables are used to ensure memory is not
   * stepped on.
   *
   * @param v0 double[]
   * @param v1 double[]
...
//		return sqrt(u[0] * u[0] + u[1] * u[1] + u[2] * u[2]) / 2.;
   ...
   * @param transform the 3 x 3 transform matrix obtained by calling
   ...
   * starting with the axis z and Z overlapping, the position of the second can be specified in
   * terms of the first using three rotations with angles A, B, C as follows:
   *
   * <ol>
   ...
   * @param radius the radius of the small circle, in radians
