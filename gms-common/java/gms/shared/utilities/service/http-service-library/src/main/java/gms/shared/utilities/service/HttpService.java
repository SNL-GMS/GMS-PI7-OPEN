...
public class HttpService {
  ...
  public void start() {
    ...
    configureServer();
    // register routes
    logger.info("Registering " + this.definition.getRoutes().size() + " routes");
    ...
    // start the service
    logger.info("Starting the sevice...");
    this.sparkService.awaitInitialization();
    isRunning = true;
    logger.info("Service is now running on port " + this.definition.getPort());
  }
  ...
  private void configureServer() {
    ...
    logger.info("Setting port of server to " + this.definition.getPort());
    this.sparkService.port(this.definition.getPort());
    logger.info(String.format("Configuring thread pool of server; "
            + "min = %d, max = %d, idle timeout = %d",
        this.definition.getMinThreadPoolSize(),
        this.definition.getMaxThreadPoolSize(),
        this.definition.getThreadIdleTimeoutMillis()));
    this.sparkService.threadPool(this.definition.getMaxThreadPoolSize(),
        this.definition.getMinThreadPoolSize(),
        this.definition.getThreadIdleTimeoutMillis());
    // Exception handler.
    this.sparkService.exception(Exception.class, HttpService::handleServiceException);
    // HTTP custom error handlers.
    this.sparkService.notFound(HttpErrorHandlers::Http404);
    this.sparkService.internalServerError(HttpErrorHandlers::Http500);
  ...
  private static spark.Route sparkRoute(RequestHandler handler,
    ...
    return (sparkRequest, sparkResponse) -> {
      ...
      sparkResponse.status(routeResponse.getHttpStatus().getCode());
  ...
  private static void handleServiceException(
      Exception e, spark.Request request, spark.Response response) {
